...
import { PlayerConfig, Playlist, Room, Song, SongChunk, PlayerState } from '../types';
...
const VaultTunePlayer = ({ config }: { config: PlayerConfig }) => {
    ...
    const [room, joinRoom] = useState< Room | undefined>(undefined);
    ...
    const bufferRef = useRef<SourceBuffer | undefined>(undefined);
    ...
    const playerState = useContext<PlayerState>(stateContext);
    ...
    async function playSong(song: Song, customRoom?: Room) {
        if (currentlyPlaying) {
        ...
        socket.emit(`play song${isOniOS(window) ? " - iOS": ""}`, customRoom ? customRoom.id : room!.id, song.id);
        ...
        bufferRef.current = undefined; // reset bufferRef to allow new song to be played
    }
    ...
    useEffect(() => {
        ...
        socket.on('available rooms', (data: Room[]) => {
            ...
            if (playerState.room && data.find(room => room.id === playerState.room!.id)) {
                    console.log("Restoring room state:", playerState.room);
                    setLoading("Restoring room state...");
                    joinRoom(playerState.room);
                    socket.emit("join room", playerState.room.id, true);
                    setSongs(playerState.songs);
                    if (playerState.currentPlaylist) {
                        console.log("Restoring current playlist:", playerState.currentPlaylist);
                        playlist(playerState.currentPlaylist, playerState.currentlyPlaying!);
                    }

                    setCurrentlyPlaying(playerState.currentlyPlaying);
                    if (playerState.currentlyPlaying) {
                        socket.once(`song data ${playerState.currentlyPlaying.id}`, () => {
                            console.log("Song data received for currently playing song, playing it");
                            audioRef.current!.play();
                            audioRef.current!.currentTime = playerState.currentTime || 0;
                            setLoading(undefined);
                        });
                        playSong(playerState.currentlyPlaying, playerState.room);
                    }
        ...
        const disconnectListener = () => {
            ...
            joinRoom(undefined);
            console.log("Persistent state updating..");
            playerState.currentPlaylist = currentPlaylist;
            playerState.currentlyPlaying = currentlyPlaying;
            playerState.room = room;
            playerState.songs = songs;
            if (currentlyPlaying) {
                playerState.currentTime = audioRef.current ? audioRef.current.currentTime : 0;
            }
        ...
        return () => {
            ...
            socket.removeAllListeners('new song');
            socket.removeAllListeners('song removed');
                            ...
                            {songs.map((song) => {
                                ...
                                return (
                                    <div className={`player-list-item ${nonPendingSong && nonPendingSong .id === song.id || pendingSong && pendingSong.id !== currentlyPlaying?.id && pendingSong.id === song.id ? 'clicked' : ''}`} key={song.id} onClick={() => {
                                        ...
                                        setPlaylistView(null); // close playlist view when a song is played
                                        if (currentPlaylist) {
                                            setCurrentPlaylist(undefined); // reset current playlist when a song is played
                                        }
                                        if (nextUp) {
    ...
    return loading ? <Loading text={loading}/> : room !== undefined ? ( songs && songs.length > 0 ? player : <Loading text='Getting songs...'  />) : roomsRef.current.length == 0 ? <Loading text="Getting rooms.." /> : (
