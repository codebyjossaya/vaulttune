...
export default function CreatePlaylist({editing} : {editing? : {
    playlist: Playlist;
}}) {
    const socketCtx = useContext(SocketContext);
    const overlayCtx = useContext(OverlayContext)
    const [open, setOpen] = useState(false);
    const [playlistName, setPlaylistName] = useState<string>(editing?.playlist.name || "");
    ...
    useEffect(() => {
      if (editing && (!playlistName || songsSelected.length === 0)) {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setPlaylistName(editing.playlist.name);
        const songsInPlaylist = editing.playlist.songs.map(songId => socketCtx?.data.songs.find(song => song.id === songId)).filter((s): s is Song => s !== undefined);
        setSongsSelected(songsInPlaylist);
      }
    }, [])
    return loading ? (
        <Loading message="Creating playlist..." />
    ) : (
            <div>
              <p>{
                  editing ? "Edit your playlist details below." : "Create a new playlist by filling out the details below."
                }</p>
              <Label htmlFor="playlist-name" className="mt-4 mb-2">Playlist Name</Label>
              <Input type="text" id="playlist-name" placeholder="Playlist Name" value={playlistName} onChange={(e) => {
              ...
              <Button className="mt-6 w-full" onClick={() => {
                ...
                if (editing) {
                  ...
                  socketCtx?.socket?.current?.emit("edit playlist", playlistName, songsSelected.map(s => s.id), editing.playlist.id);
                } else {
              ...
              }}>{editing ? "Edit Playlist" : "Create Playlist"}</Button>
        </div>
    );
